...
- CA roots are **embedded** in the binary via `src/certs.h` — generated by
  ...
  The root list covers OpenRouter (GTS/ISRG) and api.github.com
  (USERTrust/Sectigo, for the update check) — a new TLS host may need a new
  root added to `mkcerts.sh`.
